...
void generateBezierTriangles( Model *model, View *view, Projection *projection )
{
  ...
  for (int p = 0; p < model->m_numPatches; ++p) {
    int res2 = p * resolution * resolution;
    for (int row = 0; row < resolution - 1; ++row) {
      for (int col = 0; col < resolution - 1; ++col) {
        ...
        cullProjectDumpTriangle(view
                                , projection
                                , &model->m_patchVertex[res2 + row * resolution + col]
                                , &model->m_patchVertex[res2 + row * resolution + col + resolution]
                                , &model->m_patchVertex[res2 + row * resolution + col + resolution + 1]);

        cullProjectDumpTriangle(view
                                , projection
                                , &model->m_patchVertex[res2 + row * resolution + col + resolution + 1]
                                , &model->m_patchVertex[res2 + row * resolution + col + 1]
                                , &model->m_patchVertex[res2 + row * resolution + col]);
      }
    }
  }
